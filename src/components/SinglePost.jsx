...
const SinglePost = ({filterData}) => {
    const post = useFilteredPosts({ slug: filterData });
    ...
    <div>
        <div className="flex gap-4 flex-row mt-4 px-4"> 
            <Link href={post?.category?.url}>
                <Badge className="bg-blue-50 text-blue-700">{post?.category?.name}</Badge>
            </Link>
        </div>
        <div className="px-4">
            <h1 className="max-w-4xl text-3xl/tight font-semibold tracking-tight text-pretty md:text-4xl/tight lg:text-[2.5rem]/tight my-5">{post.title}</h1>
            <p className="text-base/relaxed text-neutral-600 md:text-lg/relaxed dark:text-neutral-400">{post.shortDescription}</p>
            ...
            <div className="postImage w-full h-[250px] relative rounded-2xl overflow-hidden">
                <Image src={post?.postImage?.url} alt="Blog Post" fill priority className="object-cover"/>
            </div>
